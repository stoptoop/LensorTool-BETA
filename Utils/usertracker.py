...
def usernametracker():
    try:
        ...
        username = input(f"{now_time()}~ Username ~ {purple}[ {end}> {purple}]  {end}")

        username = username.lower()

        print(colored_yellow(f"Scanning..."))
        ...
        for site, url_template in sites.items():
            try:
                ...
                try:
                    ...
                    if found == True:
                        print(f"{now_time()} {site}: {white + url}")
                    elif found == False:
                        print(f"{now_time()} {site}:{white} Not Found")

                except Exception as e:
                    print(f"{now_time()} {site}: {white + e}")
            except: 
                pass

        print(f"\n{now_time()} Total Found:{reset}")
        for site_and_url_found in sites_and_urls_found:
            time.sleep(0.5)
            print(f"{site_and_url_found}")

        print(f"\n{now_time()} Total Website: {white}{number_site}{red} Total Found: {white}{number_found}{red}")
